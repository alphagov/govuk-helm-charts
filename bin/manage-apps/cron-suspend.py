...
# ///
"""
Script to suspend cron tasks for apps in a YAML configuration file.
Adds 'suspend: true' to all cronTasks items for apps in the provided list.
...
def process_yaml_file(yaml_file_path, apps_to_suspend):
    """
    Process the YAML file and suspend cron tasks for specified apps.

    This function reads the file line by line, tracking which app context we're in,
    and adds 'suspend: true' to each cron task item for apps that should have tasks suspended.
    ...
    # Pattern to match "schedule:" line
    schedule_pattern = re.compile(r'^(\s*)schedule:\s+')
    # Pattern to match "suspend:" line (to avoid adding duplicates)
    suspend_pattern = re.compile(r'^(\s*)suspend:\s+')
    ...
    for i, line in enumerate(lines):
        ...
        if in_cron_section:
            ...
            if schedule_match and waiting_for_schedule and in_task_item:
                schedule_indent = schedule_match.group(1)

                # Check if we should suspend tasks for this app
                if current_app and current_app in apps_to_suspend:
                    # Look ahead to see if there's already a suspend line
                    has_suspend = False
                    if i + 1 < len(lines):
                        next_line = lines[i + 1]
                        if suspend_pattern.match(next_line):
                            has_suspend = True

                    if not has_suspend:
                        # Output the schedule line first
                        output_lines.append(line)
                        # Then add suspend: true line with same indentation as schedule
                        suspend_line = f"{schedule_indent}suspend: true\n"
                        output_lines.append(suspend_line)
                        print(f"Suspending cron task '{task_name}' for app: {current_app}", file=sys.stderr)
                        waiting_for_schedule = False
                        continue

                # If not suspending, just output the line normally
                output_lines.append(line)
                waiting_for_schedule = False
                continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Suspend cron tasks for apps in a YAML config file by adding 'suspend: true' to cronTasks items."
    )
    parser.add_argument(
        'yaml_file',
    ...
    # Load the list of apps to suspend cron tasks for
    apps_to_suspend = load_apps_list(args.apps_file)
    print(f"Loaded {len(apps_to_suspend)} apps from {args.apps_file}", file=sys.stderr)

    # Process the YAML file
    process_yaml_file(args.yaml_file, apps_to_suspend)
